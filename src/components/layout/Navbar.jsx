        ...
        {user ? (
          <>
            <div className="relative group flex items-center gap-2 py-2 cursor-pointer">
              <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-teal-600 border-2 border-teal-100 flex items-center justify-center text-white text-[14px] md:text-[15px] font-bold flex-shrink-0 shadow-sm transition-transform group-hover:border-teal-400">
                {user.fullName?.charAt(0).toUpperCase() || "U"}
              </div>
              <div className="hidden md:block leading-tight pr-2">
                <div className="text-[13px] font-bold text-teal-900">{user.fullName}</div>
              ...
              <div className="absolute top-full right-0 mt-2 w-72 md:w-80 bg-white border border-teal-50 shadow-2xl rounded-2xl p-4 md:p-5 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top-right group-hover:translate-y-0 translate-y-4 z-[60]">
                <div className="flex items-center gap-3 border-b border-gray-50 pb-3 mb-4">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 leading-none mb-1">{user.fullName}</h4>
                    <p className="text-[11px] text-gray-400 italic">Store Administrator</p>
                  </div>
                </div>
                <div className="space-y-3 md:space-y-4">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-teal-500 font-bold uppercase tracking-widest mb-0.5">Contact Email</span>
                    <span className="text-xs md:text-sm text-gray-700 truncate font-medium">{user.email}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-teal-500 font-bold uppercase tracking-widest mb-0.5">Business Name</span>
                    <span className="text-xs md:text-sm text-gray-700 font-medium">{user.storeName}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-teal-500 font-bold uppercase tracking-widest mb-0.5">GST Identification</span>
                    <span className="text-xs font-mono text-teal-700 bg-teal-50/50 px-2 py-1 rounded-lg border border-teal-100 inline-block w-fit">{user.gstNumber}</span>
                  </div>
                </div>
              </div>
            </div>
