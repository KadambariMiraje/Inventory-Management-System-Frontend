...
export default function ExpiringItems() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError('');
    try {
      // TODO: update endpoints when shared
      const res = tab === 'expiring'
        ? await batchAPI.getExpiring(days)
        : await batchAPI.getExpired();
      setItems(res.data || []);
  ...
  useEffect(() => {
    ...
    setFiltered(items.filter(b =>
      ...
      b.product?.productName?.toLowerCase().includes(q)
    ));
  }, [search, items]);
  ...
  const expiredCount  = items.filter(b => daysUntilExpiry(b.expiryDate) < 0).length;
  const criticalCount = items.filter(b => { const d = daysUntilExpiry(b.expiryDate); return d !== null && d >= 0 && d <= 7; }).length;
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
        ...
        {/* Days selector — only for expiring tab */}
        {tab === 'expiring' && (
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 focus:outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-400 transition-all">
      ...
      {fetching ? (
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-200">
          ...
          <p className="font-bold text-base text-slate-700">
            {search ? 'No batches match your search.' : tab === 'expiring' ? `No batches expiring within ${days} days.` : 'No expired batches.'}
          </p>
          <p className="text-sm text-slate-400 mt-1">
            {!search && tab === 'expiring' && 'Your inventory is looking fresh!'}
          </p>
        </div>
      ) : (
            ...
            <table className="w-full min-w-[640px] border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((batch, idx) => {
                  ...
                  const productName = batch.productName || batch.product?.productName || '—';

                  return (
                    <tr key={batch.batchNumber ?? idx}
                      ...
                      <td className="px-4 py-3">
                        <span className="font-mono text-sm font-bold text-teal-700 bg-teal-50 px-2 py-1 rounded-lg">
                          {batch.batchNumber}
                        </span>
                      </td>
